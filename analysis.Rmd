...
mf3 <- merge(dplyr::select(mf2, ID, type, morph), df2, by="ID")
mf4 <- data.frame(dplyr::select(mf3, ID, morph, type, treatment, line, colony), mutations=rowSums(dplyr::select(mf3, -ID, -morph, -type, -treatment, -line, -colony))) %>% filter(treatment!="ANCESTOR") %>% droplevels
```
