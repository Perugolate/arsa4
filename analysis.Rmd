...
df2 <- separate(df1, line, into = c("treatment", "line", "colony"),
		sep = "-", extra = "merge")
```
...
gpoint <- ggplot(df6p, aes(x = treatment, y = m_mu)) + geom_point(size = 5) +
  geom_errorbar(aes(ymin = m_mu - ci, ymax = m_mu + ci), width = 0.2) +
  ylab("# mutations")
gbox <- ggplot(df5, aes(x = treatment, y = mutations)) + geom_boxplot() +
  ylab("# mutations")
...
ggplot(df6p, aes(x = treatment, y = m_mu)) + geom_point(size = 5) +
  geom_errorbar(aes(ymin = m_mu - ci, ymax = m_mu + ci), width = 0.2) +
  ylab("# mutations")
dev.off()
```
...
## *ytr*/*gra* mutations
```{r}
df7 <- data.frame(dplyr::select(df4, treatment, line), ytr = rowSums(select(df4, ytrA, ytrB)), gra = rowSums(select(df4, graS, graR)), vra = rowSums(select(df4, vraS)))
df8 <- group_by(df7, treatment) %>% summarize(f_ytr = sum(ytr)/n(), f_gra = sum(gra)/n(), f_vra = sum(vra)/n()) %>% gather(operon, freq, -treatment)
