...
p_od1c <- ggplot(gdf1c, aes(x=sel, y=odM, color=morph, group=morph)) + geom_point(size=5, position=position_dodge(width = .5)) + geom_errorbar(aes(ymin=odM-od_ci, ymax=odM+od_ci), width=0.2, position=position_dodge(width = .5)) + ylab("max OD") + xlab("selection") + theme(legend.justification=c(1,0), legend.position="right")
plot_grid(p_vmax1c, p_lag1c, p_od1c, labels = c("A", "B", "C"), align = "h", ncol=3)
```
...
p_od3c <- ggplot(df2c, aes(x=sel, y=od, color=morph, group=as.factor(ID))) + geom_point(size=5, position=position_dodge(width = .5)) + ylab("max OD") + xlab("selection") + theme(legend.justification=c(1,0), legend.position="right")
```
